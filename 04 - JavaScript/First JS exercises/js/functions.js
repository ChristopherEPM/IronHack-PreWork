...
function dniLetter( dni ) {
	...
	var result = '';
	if(dni %1 !=0 || dni<0 || dni.toString().length < 8){
...
function findLongestWord( array_words ){
	...
	for(var i=0;i< array_words.length;i++){
		if(array_words[i].length >tamaño){
			tamaño=array_words[i].length;
		}
	}
	return tamaño;
...
array_lengths.push( findLongestWord( ["Mario", "Sonic", "Pacman","Rayman","Link"] ) );
array_lengths.push( findLongestWord( ["bowser", "Dr.Egman", "Piky","Evil Hand","Ganondorf"] ) );
array_lengths.push( findLongestWord( ["Luigi", "Tails"] ) );
array_lengths.push( findLongestWord( ["Mario", "Sonic", "Pacman","Rayman"] ) );
array_lengths.push( findLongestWord( ["Homer", "Bart", "Flanders","Lenny","Carl","mohe"] ) );
...
function charFreq( string ) {
	...
	for(var i=0;i< string.length;i++){
		...
		console.log(freqCounter[string[i]]);
		freqCounter[string[i]] = ++freqCounter[string[i]] || 1;// sino obtendriamos un NaN (not a number)
...
function beautifyLetters( array )
{
	...
	for(var i=0;i<array.length;i++){
		if(isFinite(array[i])==false && array[i]=="a"||array[i]=="e"||array[i]=="i"||array[i]=="o"||array[i]=="u"){
			array[i]=array[i].toUpperCase();
		}
		else if(isFinite(array[i])==false && array[i]!="a"&& array[i]!="e" && array[i]!="i" && array[i]!="o" && array[i]!="u" && array[i]!="A"&& array[i]!="E" && array[i]!="I" && array[i]!="O" && array[i]!="U"){
		...
		if (isFinite(array[i]) === true) {
			if(array[i].toString().length > 1){
				while (array[i].toString().length > 1) {
					total=0;
					arrayDeNumeros = array[i].toString().split("").map(function(t){return parseInt(t)}) //array[i].toString(10).split("") para array de caracteres
					for(var x = 0; x < arrayDeNumeros.length; x++){
						total += arrayDeNumeros[x];
					}
					array[i] = total;
				}
			}
...
{	
	...
	letras.sort(function (a, b) {
		var l=a.toLowerCase(), m=b.toLowerCase();
